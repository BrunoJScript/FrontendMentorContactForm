...
const queryOptionsContainer = Array.from(
  document.querySelectorAll(".query-option")
);
...
const isValidEmail = (value) => {
  const emailRegex =
    /^[a-zA-Z0-9]*[\._]?[a-zA-Z0-9]*@{1}[a-zA-Z]*_?[a-zA-Z]*\.{1}com$/;
...
const selectQueryType = (index) => {
  if (queryOptionsErrorText.style.display == "block")
    queryOptionsErrorText.style.display = "none";
...
const resetQueryType = () => {
  const selectedQueryType = queryOptionsContainer.find(
    (container) => container.style.backgroundColor == "lightgray"
  );
...
const inputHandler = (errorText, e) => {
  if (
    ...
    if (
      inputNotEmpty(e.currentTarget.value) &&
      e.currentTarget.style.border == "1px solid rgb(215, 60, 60)"
    ) {
...
const handleFormSubmit = (e) => {
  ...
  if (!("query-type" in data))
    handleErrorObject("Please select a query type", "query-type", errorsObject);
...
firstNameInput.addEventListener(
  "keyup",
  inputHandler.bind(null, firstNameErrorText)
);

lastNameInput.addEventListener(
  "keyup",
  inputHandler.bind(null, lastNameErrorText)
);
...
messageInput.addEventListener(
  "keyup",
  inputHandler.bind(null, messageErrorText)
);

checkboxInput.addEventListener(
  "click",
  inputHandler.bind(null, checkboxInputErrorText)
);

queryOptions.forEach((option, index) =>
  option.addEventListener("focus", selectQueryType.bind(null, index))
);
